...
function marketCapTilt(momentum, fund) {
  ...
  // Derived from the disclosed book against NSE index membership. Preferred over the SEBI
  // mandate fallback because it reflects what the fund actually holds, and it is the only
  // option for sectoral and thematic schemes, whose category implies no cap split at all.
  const derived = momentum?.snapshot?.derivedMarketCap;
  if (derived && finite(derived.largePct) && (derived.classifiedWeightPct ?? 0) >= 50) {
    return {
      largePct: derived.largePct,
      midPct: derived.midPct,
      smallPct: derived.smallPct,
      riskTilt: (derived.midPct * 0.5 + derived.smallPct),
      basis: 'index-classified',
      classifiedWeightPct: derived.classifiedWeightPct
    };
  }
...
function opportunityFactors({ momentum, metrics, peerData, selectedProxy, fund }) {
  ...
  byKey.factorAlignment = buildPillar({
    ...
    detail: () => `A ${tilt ? `${tilt.largePct.toFixed(0)}/${tilt.midPct.toFixed(0)}/${tilt.smallPct.toFixed(0)} large/mid/small` : 'an unreported market-cap'} profile and ${finite(metrics?.beta) ? metrics.beta.toFixed(2) : '—'} beta assessed against the observed ${momentum?.regime?.label || 'unclassified'} regime. ${tilt?.basis === 'sebi-mandate'
      ? 'The market-cap split is inferred from the SEBI category mandate because the source did not report one.'
      : tilt?.basis === 'index-classified'
        ? `The market-cap split is derived by classifying ${Math.round(tilt.classifiedWeightPct || 0)}% of the disclosed book against NSE index membership, because the source did not report one.`
        : 'The market-cap split is the reported portfolio breakdown.'} Regime classification is momentum-based and descriptive, not a forecast.`,
