...
export async function archiveSnapshot(schemeCode, snapshot) {
  ...
  // A richer disclosure (more holdings, or one carrying ISINs) supersedes a thinner one for
  // the same month; otherwise the first observation is kept so archives stay stable.
  const incomingQuality = holdings.length + (holdings.some(item => item.isin) ? 1000 : 0);
  const existingQuality = existing ? existing.h.length + (existing.h.some(row => row.i) ? 1000 : 0) : -1;
