...
function identityScore(value, fund) {
  return Math.max(0, ...expectedNames(fund).map(name => incompatibleStyle(name, value) ? 0 : score(name, value)));
}
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'user-agent': 'ManagerLens/1.2 Value Research fund identity resolver'
      }
    });
...
function currentIdentity(research, fund) {
  ...
  const styleMismatch = expectedNames(fund).every(name => incompatibleStyle(name, value));
...
async function discoverPage(fund) {
  ...
  if (!best) return { ok: false, error: 'No sufficiently close Value Research fund page was found.', stage: 'fund-page-match' };
  return { ok: true, url: best.url, label: best.text, score: best.score, houseUrl: house.url };
}

export async function repairValueResearchFundMatch(research, fund) {
