...
import { archiveSnapshot, previousSnapshot } from '@/lib/portfolio-archive';
...
export async function resolveFundResearch(fund) {
  ...
  if (archiveKey && current.holdings?.length) {
  ...
  const value = {
    ok: Boolean(valueResearch || moneycontrol || advisorKhoj),
    current,
    previous,
