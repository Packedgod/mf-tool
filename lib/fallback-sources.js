...
import { enrichMoneycontrolLookThrough, fetchMoneycontrolPortfolio } from '@/lib/moneycontrol-portfolio';
import { archiveSnapshot, previousSnapshot, bestRecentSnapshot } from '@/lib/portfolio-archive';
...
export async function resolveFundResearch(fund) {
  ...
  const [vroPage, moneycontrolResult] = await Promise.all([
    vroUrl ? fetchText(vroUrl, 20000, true) : Promise.resolve(null),
    fetchMoneycontrolPortfolio(fund)
  ]);
  const moneycontrol = moneycontrolResult?.ok ? moneycontrolResult : null;
  const liveAdvisor = moneycontrol?.holdings?.length || moneycontrol?.sectors?.length
    ? null
    : await fetchAdvisorKhoj(fund, storedAdvisor?.url);
  ...
  const portfolioAsOf = valueResearch?.holdings?.length
    ? valueResearch.navDate || valueResearch.fetchedAt?.slice(0, 10)
    : moneycontrol?.holdings?.length || moneycontrol?.sectors?.length || moneycontrol?.assetAllocation?.length
      ? moneycontrol.fetchedAt?.slice(0, 10)
      : advisorKhoj?.navDate || null;
  ...
  let current = {
    ...
    turnoverPct: valueResearch?.turnoverPct ?? moneycontrol?.turnoverPct ?? advisorKhoj?.turnoverPct ?? generated?.current?.turnoverPct,
    benchmark: valueResearch?.benchmark || advisorKhoj?.benchmark || generated?.current?.benchmark,
    holdings: valueResearch?.holdings?.length ? valueResearch.holdings : moneycontrol?.holdings?.length ? moneycontrol.holdings : advisorKhoj?.holdings?.length ? advisorKhoj.holdings : generated?.current?.holdings || [],
    sectors: moneycontrol?.sectors?.length
      ? moneycontrol.sectors
      : moneycontrol?.assetAllocation?.length
        ? moneycontrol.assetAllocation.map(item => ({ sector: `Asset allocation: ${item.name}`, weight: item.weight, allocationProxy: true }))
        : advisorKhoj?.sectors?.length
          ? advisorKhoj.sectors
          : generated?.current?.sectors || [],
    assetAllocation: valueResearch?.assetAllocation?.length ? valueResearch.assetAllocation : moneycontrol?.assetAllocation?.length ? moneycontrol.assetAllocation : generated?.current?.assetAllocation || [],
    marketCap: valueResearch?.marketCap?.length ? valueResearch.marketCap : moneycontrol?.marketCap?.length ? moneycontrol.marketCap : generated?.current?.marketCap || [],
    expenseRatioPct: valueResearch?.expenseRatioPct ?? moneycontrol?.expenseRatioPct ?? advisorKhoj?.expenseRatioPct ?? generated?.current?.expenseRatioPct,
    aumCr: valueResearch?.aumCr ?? moneycontrol?.aumCr ?? advisorKhoj?.aumCr ?? generated?.current?.aumCr,
    riskMetrics: moneycontrol?.riskMetrics || advisorKhoj?.riskMetrics || generated?.current?.riskMetrics || null,
    sources: [
      valueResearch && { name: 'Value Research Online', type: liveVro ? 'Live manager, holdings, turnover and fund facts' : 'Fund identity and manager metadata from the source-tracked registry', url: valueResearch.url, asOf: valueResearch.navDate || valueResearch.fetchedAt },
      moneycontrol && { name: 'Moneycontrol', type: 'Identity-validated live holdings, allocation and fund facts', url: moneycontrol.url, asOf: moneycontrol.fetchedAt },
  ...
  // Bank this month's disclosure, then fall back to the durable archive when no second
  // dated snapshot came from the live sources. The in-memory cache above only survives the
  // process, so without this a fund almost never accumulates comparable history.
  const archiveKey = fund.preferredSchemeCode || fund.schemeCode || fund.id;
  if (archiveKey) {
    if (current.holdings?.length) {
      await archiveSnapshot(archiveKey, {
        asOf: current.portfolioAsOf || current.fetchedAt,
        holdings: current.holdings,
        source: current.officialPortfolio?.source?.name || current.sources?.[0]?.name || 'Resolved disclosure',
        url: current.officialPortfolio?.source?.url || current.sources?.[0]?.url || null
      }).catch(() => null);
    }

    // Upstream sources degrade without warning: when Value Research throttles a request the
    // pipeline silently falls back to a handful of lines — sometimes to nothing at all — and
    // that thin book then gets cached and served for the next fifteen minutes. Every
    // concentration, Active Share and holdings-quality factor drops to Not Rated as a
    // result, even though a complete disclosure for the same month is already archived.
    //
    // The restore deliberately sits outside the archiving branch above: the worst
    // degradations return an empty book, which is exactly when the archive matters most.
    const archivedBest = await bestRecentSnapshot(archiveKey).catch(() => null);
    const liveWeight = (current.holdings || []).reduce((sum, item) => sum + (Number(item.weight) || 0), 0);
    if (archivedBest && archivedBest.weight > liveWeight + 5) {
      current = {
        ...current,
        holdings: archivedBest.holdings,
        portfolioAsOf: archivedBest.asOf || current.portfolioAsOf,
        restoredFromArchive: {
          month: archivedBest.month,
          source: archivedBest.source,
          liveWeightPct: Number(liveWeight.toFixed(1)),
          archivedWeightPct: Number(archivedBest.weight.toFixed(1))
        },
        sources: [
          ...(current.sources || []),
          {
            name: `${archivedBest.source || 'Archived disclosure'} (locally archived ${archivedBest.month})`,
            type: 'Complete portfolio restored from the local month-end archive after the live source returned a partial book',
            url: archivedBest.url
          }
        ]
      };
    }
    ...
    if (!previous?.holdings?.length) {
      const archived = await previousSnapshot(archiveKey, current.portfolioAsOf || current.fetchedAt).catch(() => null);
      if (archived?.holdings?.length) {
        previous = {
          holdings: archived.holdings,
          sectors: [],
          portfolioAsOf: archived.asOf,
          fetchedAt: archived.asOf,
          sources: [{ name: `${archived.source || 'Archived disclosure'} (locally archived ${archived.month})`, type: 'Archived month-end portfolio disclosure', url: archived.url }]
        };
      }
    }
  }
  const value = {
    // A restored archive is a real, complete disclosure, so it counts as a resolved source.
    // Without this the route 404s whenever every live source fails at once, even though the
    // app is holding a full month-end book for the fund.
    ok: Boolean(valueResearch || moneycontrol || advisorKhoj || current.restoredFromArchive),
    current,
    previous,
    ...
    diagnostics: {
      ...
      liveMoneycontrol: Boolean(moneycontrol),
      ...
      moneycontrolError: moneycontrolResult && !moneycontrolResult.ok ? moneycontrolResult.error : null,
      moneycontrolIdentity: moneycontrolResult?.identity || null,
      moneycontrolCoverage: moneycontrol?.coverage || null
    },
...
export async function enrichFundResearchLookThrough(research, fund, families = []) {
  const parent = research?.current?.moneycontrol;
  if (!research?.ok || !parent?.ok) return research;
  const moneycontrol = await enrichMoneycontrolLookThrough(fund, parent, families);
  if (!moneycontrol?.isFundOfFunds) return research;
  return {
    ...research,
    current: {
      ...research.current,
      moneycontrol,
      holdings: moneycontrol.holdings || research.current.holdings || [],
      sectors: moneycontrol.sectors || research.current.sectors || [],
      directHoldings: moneycontrol.directHoldings || [],
      underlyingFunds: moneycontrol.underlyingFunds || [],
      lookThrough: {
        enabled: Boolean(moneycontrol.lookThroughHoldings?.length),
        coverage: moneycontrol.lookThroughCoverage || null,
        source: 'Moneycontrol portfolios for AMFI-matched underlying schemes'
      }
    },
    diagnostics: {
      ...(research.diagnostics || {}),
      moneycontrolLookThrough: moneycontrol.lookThroughCoverage || null
    }
  };
}
