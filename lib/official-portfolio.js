...
function classifyHeader(value) {
  ...
  // Debt disclosures put the credit rating in the same column as the equity industry
  // ("Industry / Rating"), so an explicit rating header is matched before that fallback.
  if (/^rating|credit rating|rating\s*\/|\brating\b(?!.*industry)/.test(text)) return 'rating';
  if (/industry|sector|classification/.test(text)) return 'sector';
  if (/ytm|yield to maturity|annualis?ed yield|yield\b/.test(text)) return 'ytm';
  if (/%.*nav|nav.*%|percentage.*nav|%.*net asset|weight.*%|portfolio.*%|% of net/.test(text)) return 'weight';
  if (/market\s*value|market\/fair value|fair value|amount.*lakh|value.*lakh|value.*cr/.test(text)) return 'marketValue';
  if (/quantity|no\.? of shares|units held|no\.? of units/.test(text)) return 'quantity';
...
// SEBI debt disclosures report ratings as "CRISIL AAA", "[ICRA]AA+", "SOVEREIGN", "A1+".
// Everything is reduced to the rating token itself so credit quality can be scored.
function normaliseRating(value) {
  const text = cleanText(value).toUpperCase();
  if (!text) return null;
  if (/SOVEREIGN|\bSOV\b|\bGOI\b|\bG-?SEC\b/.test(text)) return 'SOVEREIGN';
  if (/UNRATED|NOT RATED/.test(text)) return 'UNRATED';
  // A trailing \b cannot match after "+" or "-" (neither is a word character), which would
  // silently collapse AA+ into AA and merge distinct credit tiers. A negative lookahead is
  // used instead so the modifier survives, while still refusing to match the leading letter
  // of an ordinary word such as "Banks" or "Cement".
  const shortTerm = text.match(/\bA([1-4])\s*(\+?)(?![A-Z0-9])/);
  if (shortTerm) return `A${shortTerm[1]}${shortTerm[2]}`;
  const longTerm = text.match(/\b(AAA|AA|A|BBB|BB|B|C|D)\s*([+-]?)(?![A-Z0-9])/);
  if (longTerm) return `${longTerm[1]}${longTerm[2]}`;
  return null;
}
...
function parseSheet(rows, sheetName) {
  ...
  for (let index = rowIndex + 1; index < Math.min(rows.length, rowIndex + MAX_ROWS_PER_SHEET); index += 1) {
    ...
    // Where a combined "Industry / Rating" column is used, the rating lives in the sector
    // cell, so it is read from there when no dedicated rating column was found.
    const rating = normaliseRating(
      mapping.rating !== undefined ? row[mapping.rating] : (mapping.sector !== undefined ? row[mapping.sector] : null)
    );
    holdings.push({
      name,
      sector,
      ...
      isin: isin || null,
      rating,
      ytmPct: mapping.ytm !== undefined ? numberValue(row[mapping.ytm]) : undefined,
      quantity: mapping.quantity !== undefined ? numberValue(row[mapping.quantity]) : undefined,
      marketValue: mapping.marketValue !== undefined ? numberValue(row[mapping.marketValue]) : undefined,
      sheetName
    });
  }
