...
import { recordUniverseObservation } from "@/lib/universe-history";
...
function isAdministrativeScheme(name) {
  const text = String(name || '');
  return /\bunclaimed\b|withheld\s+brokerage|segregated\s+portfolio/i.test(text)
    || /\bplan\s+c\b.*\b(?:idcw|redemption)\b.*\b(?:above|up\s*to|upto)\s*3\s*y/i.test(text);
}
...
function parseAmfiUniverse(text) {
  ...
  for (const rawLine of String(text || "").split(/\r?\n/)) {
    ...
    if (/^\d+;/.test(line)) {
      ...
      if (!schemeCode || !schemeName || isAdministrativeScheme(schemeName)) continue;
      schemes.push({
        schemeCode,
        schemeName,
...
function buildFamilies(schemes) {
  ...
  return [...groups.values()].map(group => {
    ...
    return {
      ...
      preferredIsin: [preferred.isinGrowth, preferred.isinDividend].find(value => /^INF[0-9A-Z]{9}$/i.test(String(value || '').trim())) || null,
...
export async function getMarketUniverse() {
  ...
  try {
    ...
    // Bank today's membership so point-in-time peer sets (including funds that later merge
    // or close) become available over time. Never blocks or fails the response.
    recordUniverseObservation(schemes).catch(() => {});
...
export function searchUniverse(universe, { query = "", fundHouse = "", managerOnly = false, limit = 100, offset = 0 } = {}) {
  ...
  const filtered = universe.families.filter(fund => {
    ...
    return [
      ...
      fund.fundHouse,
      fund.category,
      fund.preferredSchemeCode,
      ...fund.variants.flatMap(variant => [variant.schemeCode, variant.schemeName]),
      ...fund.managers.flatMap(manager => [manager.name, manager.role])
    ].join(" ").toLowerCase().includes(q);
  });
  if (q) {
    const score = fund => {
      const displayName = String(fund.displayName || "").toLowerCase();
      const canonicalName = String(fund.canonicalName || "").toLowerCase();
      const fundHouseName = String(fund.fundHouse || "").toLowerCase();
      const exactCode = fund.variants.some(variant => String(variant.schemeCode) === q);
      const exactManager = fund.managers.some(manager => String(manager.name || "").toLowerCase() === q);
      if (exactCode) return 10000;
      if (displayName === q || canonicalName === q) return 9000;
      if (displayName.startsWith(q) || canonicalName.startsWith(q)) return 8000;
      if (exactManager) return 7500;
      if (fundHouseName === q) return 7000;
      if (fundHouseName.startsWith(q)) return 6000;
      return 1000;
    };
    filtered.sort((a, b) => score(b) - score(a) || a.displayName.localeCompare(b.displayName));
  }
