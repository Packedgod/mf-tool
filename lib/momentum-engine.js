...
function scoreSectorMovement(snapshot, market) {
  ...
  if (!history.length) {
    ...
    return {
      ...
      available: false,
      value: bias.value,
      detail: "No multi-month sector-weight history is available in the latest official factsheet; current positioning is used as a provisional signal."
    };
  }

  const timingRows = history.map(item => {
    const values = item.values || [];
    const change = values.length >= 2 ? values.at(-1) - values.at(-2) : 0;
...
function scoreEntries(market) {
  ...
  if (!resolved.length) return { score: 50, available: false, rows: market?.entries || [], detail: "No price-resolved stock entries are available for this factsheet period." };
...
function scoreExits(market) {
  ...
  if (!resolved.length) return { score: 50, available: false, rows: market?.exits || [], detail: "No price-resolved stock exits are available for this factsheet period." };
  const raw = average(resolved.map(item => clamp(50 - item.postEventReturnPct * 2.2)));
  return {
    score: clamp(raw),
    available: true,
    rows: market.exits,
    detail: `${resolved.length} exits were checked against price movement after the approximate monthly exit date.`
  };
}

function scoreExitPeak(market) {
  const resolved = (market?.exits || []).filter(item => item.ok && Number.isFinite(item.peakProximityPct));
  if (!resolved.length) return { score: 50, available: false, rows: market?.exits || [], detail: "Peak proximity requires a resolvable stock symbol and an exit month." };
...
function scoreTurnover(snapshot, traditional, schemeId) {
  ...
  if (!Number.isFinite(turnover)) return { score: 50, available: false, value: undefined };
  const [low, high] = turnoverBand(schemeId);
  let discipline;
  if (turnover >= low && turnover <= high) discipline = 85;
  else if (turnover < low) discipline = clamp(85 - (low - turnover) * 2.2);
  else discipline = clamp(85 - (turnover - high) * 0.45);
...
function scoreCycleFit(schemeId, market) {
  const profile = CYCLE_PROFILES[schemeId];
...
export function calculateManagerScore({ schemeId, snapshot, market, traditional }) {
  ...
  const insights = [
    Number.isFinite(factors.sectorBias.value)
      ...
      : "Sector-market alignment is waiting for live Yahoo Finance sector data.",
