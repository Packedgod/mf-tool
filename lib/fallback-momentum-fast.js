...
async function enrichHolding(holding) {
  const match = holding.symbol ? { symbol: holding.symbol, sector: holding.sector } : await searchSymbol(holding.name);
  const points = await chart(match?.symbol);
  return {
    ...holding,
    symbol: match?.symbol || holding.symbol || null,
    sector: sectorAlias(holding.sector && holding.sector !== 'Unclassified' ? holding.sector : match?.sector || 'Other'),
    ok: Boolean(points),
...
export async function buildFallbackMomentumFast(fund, research) {
  ...
  const comparisonMode = research.portfolioComparison?.mode || 'top-holdings-proxy';
  const eventDate = current.portfolioAsOf || current.fetchedAt?.slice(0, 10);
  const rawHoldings = (current.holdings || []).slice(0, 30);
  const enrichedAll = await Promise.all(rawHoldings.slice(0, 15).map(enrichHolding));

  let sectorWeights = current.sectors?.length ? current.sectors : [];
  let sectorBasis = current.officialPortfolio?.source
  ...
  if (!sectorWeights.length) {
    ...
    sectorBasis = 'Value Research top-holdings sector proxy with live market classifications';
  }
  const sectorStats = await Promise.all(sectorWeights.map(enrichSector));

  const books = comparableBooks(rawHoldings, previous.holdings || [], comparisonMode);
  const changes = previous.holdings?.length ? buildHoldingChanges(books.currentRows, books.previousRows, eventDate) : { entries: [], exits: [] };
  ...
  const exits = await Promise.all(changes.exits.slice(0, 15).map(async item => {
    const match = item.symbol ? { symbol: item.symbol, sector: item.sector } : await searchSymbol(item.name);
  ...
  const hasPrevious = Boolean(previous.holdings?.length);
  ...
  const portfolioSources = (current.sources || []).filter(item => /Value Research|AdvisorKhoj/i.test(item.name || item.type || ''));
  const priceSources = [
  ...
  return {
    ...
    snapshot: {
      turnover: { equityPct: current.turnoverPct, totalPct: current.turnoverPct, interpretation: 'Turnover from Value Research Online or AdvisorKhoj' },
      sectorWeights,
      sectorHistory,
      coverageNote: hasPrevious
        ...
        : 'The current Value Research/AdvisorKhoj snapshot is available; a second dated source snapshot was not returned for this fund.',
      factsheetUrl: portfolioSources[0]?.url || current.sources?.[0]?.url,
      factsheetLabel: portfolioSources.map(item => item.name).join(' + ') || 'Value Research Online + AdvisorKhoj',
      assetAllocation: current.assetAllocation || [],
      marketCap: current.marketCap || [],
      sectorBasis,
      comparisonMode,
      currentAsOf: current.portfolioAsOf || current.fetchedAt,
      previousAsOf: previous.portfolioAsOf || previous.fetchedAt,
