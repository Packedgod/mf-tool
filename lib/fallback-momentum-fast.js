...
import { indexConstituents, indexIdForBenchmark, activeShare, marketCapSplit } from '@/lib/index-constituents';
...
export async function buildFallbackMomentumFast(fund, research) {
  ...
  // Keep the whole disclosed book. A 30-row cap silently dropped ~23% of portfolio weight
  // on funds that publish a complete portfolio, which pushed concentration and Active Share
  // below their disclosure thresholds and left those factors unrated. Only the price-history
  // enrichment below stays capped, because that is the slow per-holding call; fundamentals
  // are a cheap cached lookup and run across the full book.
  const rawHoldings = (current.holdings || []).slice(0, 150);
  ...
  // Fundamentals resolve by disclosed name as well as ticker, so every holding is offered
  // rather than only the largest exposures that also cleared market-price enrichment.
  const fundamentals = await fetchFundamentals(enrichedBase);
  const enrichedAll = enrichedBase.map(item => {
    const facts = fundamentals.get(item.symbol || item.name);
    return facts?.ok ? { ...item, fundamentals: facts } : item;
  });
  ...
  // Benchmark constituent weights are resolved here, on the server, because the scoring
  // engine runs in the browser and cannot fetch them itself.
  const benchmarkIndex = await indexConstituents(
    indexIdForBenchmark(fund?.benchmark || fund?.category || current.category || '')
  );
  const share = activeShare(enrichedAll, benchmarkIndex);
  // Derived here because it needs index membership, which only the server fetches. Without
  // it, funds whose source publishes no market-cap breakdown and whose SEBI category implies
  // no mandated split (sectoral and thematic schemes) leave factor alignment unrated.
  const derivedMarketCap = await marketCapSplit(enrichedAll).catch(() => null);
  const benchmarkSectors = new Map();
  for (const member of benchmarkIndex.members || []) {
    if (!Number.isFinite(member.weightPct)) continue;
    benchmarkSectors.set(sectorAlias(member.industry), (benchmarkSectors.get(sectorAlias(member.industry)) || 0) + member.weightPct);
  }
  ...
  const priceSources = [
    ...
    { name: 'Yahoo Finance quoteSummary', type: 'Security-level valuation, profitability and growth fundamentals', asOf: new Date().toISOString() },
  ...
  return {
    ...
    snapshot: {
      ...
      // Only consulted when the source publishes no split of its own; kept separate so the
      // reported breakdown always wins and the derived one is labelled as derived.
      derivedMarketCap: derivedMarketCap || null,
      ...
      sectorBasis,
      // Surfaced so a restored book is visible in the response and the sources tab rather
      // than silently standing in for a live one.
      restoredFromArchive: current.restoredFromArchive || null,
      disclosedWeightPct: Number(rawHoldings.reduce((sum, item) => sum + (Number(item.weight) || 0), 0).toFixed(1)),
      comparisonMode,
    ...
    benchmark: {
      indexId: benchmarkIndex.id,
      label: benchmarkIndex.label,
      constituents: benchmarkIndex.coverage,
      approximate: Boolean(benchmarkIndex.approximate),
      basis: benchmarkIndex.basis,
      error: benchmarkIndex.error || null,
      activeShare: share,
      sectorWeights: [...benchmarkSectors].map(([sector, weight]) => ({ sector, weight }))
    },
    ...
    coverage: {
      ...
      fundamentalsResolved: enrichedAll.filter(item => item.fundamentals?.ok).length,
      fundamentalsWeightPct: (() => {
        const total = enrichedAll.reduce((sum, item) => sum + (Number(item.weight) || 0), 0);
        const covered = enrichedAll.filter(item => item.fundamentals?.ok).reduce((sum, item) => sum + (Number(item.weight) || 0), 0);
