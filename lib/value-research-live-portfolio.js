...
function exactBlock(text, startMarker, endMarkers) {
  ...
  return text.slice(after, end >= 0 ? end : undefined).trim();
}
...
function parseHoldings(text) {
  const section = exactBlock(text, 'Company Percentage of Portfolio', [
    'See More',
    'What is the return',
...
function parseAllocation(text, heading, endHeading, labels) {
  const section = exactBlock(text, heading, [endHeading]);
  const result = [];
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'user-agent': 'ManagerLens/1.1 Value Research exact-portfolio reader'
      }
    });
...
export async function refreshValueResearchPortfolio(research) {
  ...
  if (!page.ok) {
    return {
      ...research,
      exactValueResearchPortfolio: { ok: false, error: page.error }
    };
  }
  ...
  const navDate = text.match(/latest declared NAV[\s\S]{0,180}?as of\s+(\d{1,2}-[A-Za-z]{3}-\d{4})/i)?.[1]
