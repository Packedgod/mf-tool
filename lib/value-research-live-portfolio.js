...
function parseHoldings(text) {
  ...
  for (const match of section.matchAll(pattern)) {
    ...
    // A single position can exceed 25% in debt and hybrid schemes (one government security
    // often does), and a complete disclosure runs well past 20 lines, so neither limit
    // should discard rows the page actually published.
    if (!validCompany(name) || !Number.isFinite(weight) || weight <= 0 || weight > 100 || seen.has(key)) continue;
    seen.add(key);
...
export async function refreshValueResearchPortfolio(research) {
  ...
  // This refresh exists to make the portfolio *exact*, not to shrink it. The public fund
  // page only lists a handful of names above its "See More" control, so the parsed block is
  // routinely far thinner than the book already resolved from the portfolio page or
  // Moneycontrol. Overwriting unconditionally was discarding complete disclosures — a
  // 78-holding book covering 100% of weight was being replaced by 5 holdings covering 18% —
  // which then suppressed every concentration, Active Share and holdings-quality factor
  // downstream. The richer book wins; the refresh still contributes its metadata.
  const existingHoldings = current.holdings || [];
  const weightOf = rows => rows.reduce((sum, item) => sum + (Number(item.weight) || 0), 0);
  const refreshedWeight = weightOf(holdings);
  const existingWeight = weightOf(existingHoldings);
  const refreshedIsRicher = refreshedWeight >= existingWeight - 0.5 || holdings.length > existingHoldings.length;
  const preferredHoldings = refreshedIsRicher ? holdings : existingHoldings;

  const updatedValueResearch = {
    ...valueResearch,
    url: page.url,
    fetchedAt: page.fetchedAt,
    holdings: preferredHoldings,
  ...
  return {
    ...
    exactValueResearchPortfolio: {
      ...
      url: page.url,
      fetchedAt: page.fetchedAt,
      holdings: preferredHoldings.length,
      parsedFromFundPage: holdings.length,
      supersededByRicherBook: !refreshedIsRicher,
      totalTopHoldingWeight: Number(weightOf(preferredHoldings).toFixed(4)),
