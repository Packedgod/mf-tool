...
function managerKey(record) {
  const names = [record.name, ...(record.aliases || [])].map(slugify).filter(Boolean);
  return `${names[0] || slugify(record.id)}::${normaliseAmc(record.amc)}`;
}
