...
function dedupeRegistry(records) {
  ...
  for (const rawRecord of records) {
    ...
    merged[index] = {
      ...
      // Spreading `record` would let a source that carries no tenure blank one that does,
      // so the earliest known start date is kept explicitly.
      managingSince: [existing.managingSince, record.managingSince].filter(Boolean).sort()[0] || null,
      managingSinceInception: Boolean(existing.managingSinceInception || record.managingSinceInception),
