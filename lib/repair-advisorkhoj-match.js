...
const DIRECT_DOCUMENT = /\.(?:xlsx?|xlsm|xls|csv|pdf)(?:$|[?#])/i;
const AMC_ALIASES = [
...
function documentIdentity(documents, fund) {
  ...
  if (!documents?.length || !target) return { valid: true, targetCount: 0, foreignCount: 0, directCount: 0 };
  let targetCount = 0;
  let foreignCount = 0;
  let directCount = 0;
  for (const document of documents) {
    const source = decodeURIComponent(`${document.url || ''} ${document.text || ''}`).toLowerCase();
    if (DIRECT_DOCUMENT.test(document.url || '')) directCount += 1;
...
function hasUsablePortfolioPath(advisor, docsCheck) {
  return Boolean(
    advisor?.holdings?.length
    || advisor?.sectors?.length
    || docsCheck.directCount
  );
}
...
function currentIdentity(advisor, fund) {
  ...
  const usablePortfolioPath = hasUsablePortfolioPath(advisor, docs);
  return {
    accepted: !foreignUrl && docs.valid && usablePortfolioPath && (targetPresent || fundScore >= 0.38),
    targetPresent,
    fundScore,
    foreignUrl,
    usablePortfolioPath,
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'user-agent': 'ManagerLens/1.4 AdvisorKhoj alias resolver'
      }
    });
...
function mergeAdvisor(research, advisorKhoj, validation, attempts) {
  ...
  return {
    ...
    current: {
      ...research.current,
      advisorKhoj,
      holdings: research.current?.valueResearch?.holdings?.length
        ? research.current.valueResearch.holdings
        : research.current?.moneycontrol?.holdings?.length
          ? research.current.moneycontrol.holdings
          : advisorKhoj.holdings || [],
      sectors: research.current?.moneycontrol?.sectors?.length ? research.current.moneycontrol.sectors : advisorKhoj.sectors || [],
...
export async function repairAdvisorKhojFundMatch(research, fund) {
  ...
  if (research.current?.moneycontrol?.identity?.accepted && (research.current.moneycontrol.holdings?.length || research.current.moneycontrol.sectors?.length)) {
    return {
      ...research,
      advisorIdentityValidation: {
        accepted: false,
        repaired: false,
        skipped: true,
        reason: 'An identity-validated Moneycontrol portfolio is already available.'
      }
    };
  }
  ...
  for (const slug of slugs(fund)) {
    ...
    const usablePortfolioPath = hasUsablePortfolioPath(advisorKhoj, docsCheck);
    attempts.push({ requestedUrl, returnedUrl: page.url, ...check, ...docsCheck, usablePortfolioPath });
    if (!docsCheck.valid || !usablePortfolioPath) continue;
    return mergeAdvisor(baseResearch, advisorKhoj, {
      ...
      directDocuments: docsCheck.directCount
    }, attempts);
  }
  ...
  return {
    ...
    advisorIdentityRejection: {
      ...
      reason: 'No alias-matched AdvisorKhoj page exposed a validated holdings, sector, or direct portfolio-document path.'
    },
