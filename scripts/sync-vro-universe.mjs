...
function extractFundHouse(text, fallback) {
  const fromAbout = text.match(/mutual fund scheme of\s+(.+? Mutual Fund)\./i)?.[1]?.trim();
  const fromBasics = text.match(/Fund House\s+([^\n]+? Mutual Fund)(?:\s+Launch Date|\n|$)/i)?.[1]?.trim();
  const fromAmc = text.match(/AMC\s+([^\n]+?Asset Management[^\n]*)/i)?.[1]?.trim();
  return fromAbout || fromBasics || fromAmc || fallback || 'Unknown Mutual Fund';
}

function parsePage(html, url, fallbackName, discoveryHouse) {
  ...
  const fundHouse = extractFundHouse(text, discoveryHouse);
...
async function discoverFunds() {
  ...
  for (const house of houses) {
    ...
    for (const item of links(page.html, page.url)) {
      ...
      if (!existing || name.length > existing.name.length) funds.set(cleanUrl, { url: cleanUrl, name, discoveryHouse: house.name });
    }
  }
...
async function main() {
  ...
  console.log(`Discovered ${discovery.funds.length} candidate direct-plan fund pages across ${discovery.houses} selector pages.`);
  ...
  const parsed = await mapConcurrent(discovery.funds, CONCURRENCY, async fund => {
    ...
    const record = parsePage(page.html, page.url, fund.name, fund.discoveryHouse);
  ...
  const meta = {
    generatedAt,
    selectorPagesScanned: discovery.houses,
  ...
  console.log(`Wrote ${managers.length} managers and ${records.length} fund records across ${meta.fundHousesResolvedFromFundPages} resolved fund houses.`);
}
